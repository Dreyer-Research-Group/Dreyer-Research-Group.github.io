  ! Purpose: Program to use 2nd order RK method for a body rotating around
  ! the sun
  ! Author: Cyrus Dreyer
  ! Date: 4/18/21
program rk2_orbit
  ...
  do ii=1,n_time-1
     ...
     pos_x(ii+1)=pos_x(ii) + k2_pos_x
     pos_y(ii+1)=pos_y(ii) + k2_pos_y
     vel_x(ii+1)=vel_x(ii) + k2_vel_x
     vel_y(ii+1)=vel_y(ii) + k2_vel_y
     
  end do

  ! Make gnuplot file
  open(newunit=lun_out,file='rk2_orbit_plot.dat',status='replace')

  write(lun_out,*) "set terminal png size 640,640"
  write(lun_out,*) "set output 'rk2_orbit_plot.png'"
